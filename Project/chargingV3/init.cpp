...
{ 
	...
	//读取继续扫描标志
	m_bContinueScan = CReadIniFile::getInstance()->readProfileInfo("SET", "ContinueScan", g_AppPath + "\\set.ini", &iError).toInt()==1;
	//读取继续网络提交标志
	m_bContinueSubmit = CReadIniFile::getInstance()->readProfileInfo("SET", "ContinueSubmit", g_AppPath + "\\set.ini", &iError).toInt() == 1;
	...
	//扫描间隔时间
	m_ScanDeviceInterval = CReadIniFile::getInstance()->readProfileInfo("SET", "ScanDeviceInterval", g_AppPath + "\\set.ini", &iError).toInt();
	...
	//读取调试信息显示等级
	m_enShowDebugInfoLevel = (enDebugInfoPriority)(CReadIniFile::getInstance()->readProfileInfo("WidgetShow", "debugInfoLevel", g_AppPath + "\\set.ini", &iError).toInt());

	//读取调试信息自动隐藏标志
	m_bAutoHideDebugInfo = (CReadIniFile::getInstance()->readProfileInfo("WidgetShow", "AutoHideDebugInfo", g_AppPath + "\\set.ini", &iError).toInt()) == 1;

	m_bCanMoveWindow = (CReadIniFile::getInstance()->readProfileInfo("WidgetShow", "moveWindow", g_AppPath + "\\set.ini", &iError).toInt()) == 1;

	//读取充电时限
	m_nChargeLimitTime = (CReadIniFile::getInstance()->readProfileInfo("SET", "ChargeLimitTime", g_AppPath + "\\set.ini", &iError).toInt());
