...
{ 
	...
	setPalette(pa);
	ui.lab_logo->setStyleSheet("QLabel{border-image: url(" + g_AppPath + "/img/msg_tips.png);}");
	ui.lab_logo->setText("");
...
{ 
	...
	if (nType == 0){
		...
		ui.lab_logo->setStyleSheet("QLabel{border-image: url(" + g_AppPath + "/img/msg_warning.png);}");
		this->setWindowTitle("警告");
		ui.lab_message->setStyleSheet("QLabel{font: 14px;color:black;}");
	}
	else if (nType == 2){
		ui.lab_logo->setStyleSheet("QLabel{border-image: url(" + g_AppPath + "/img/msg_error.png);}");
