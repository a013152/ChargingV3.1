...
void charging::show_temperature(float ftemp, bool high)
{
	...
	if(ftemp < 5)
		...
		if (high || ftemp >=40)
			ui.lab_temperature_imge->setStyleSheet("QLabel{border-image: url(" +  QString::fromLocal8Bit(g_AppPath) + "/img/temperature_high.png);}");
		else
			ui.lab_temperature_imge->setStyleSheet("QLabel{border-image: url(" +  QString::fromLocal8Bit(g_AppPath) + "/img/temperature_normal.png);}");
	}
